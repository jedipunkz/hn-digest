---
source: ""
hn_url: "https://news.ycombinator.com/item?id=48544106"
title: "Ask HN: Can you stay relevant against AI via specialization"
article_title: ""
author: "WhyIsItAlwaysHN"
captured_at: "2026-06-15T19:26:47Z"
capture_tool: "hn-digest"
hn_id: 48544106
score: 2
comments: 2
posted_at: "2026-06-15T17:01:23Z"
tags:
  - hacker-news
  - translated
---

# Ask HN: Can you stay relevant against AI via specialization

- HN: [48544106](https://news.ycombinator.com/item?id=48544106)
- Score: 2
- Comments: 2
- Posted: 2026-06-15T17:01:23Z

## Translation

タイトル: HN に聞く: 専門化を通じて AI との関連性を維持できますか
HN テキスト: 私は、AI エージェントによって私たちが置き換えられる可能性と、AI インテリジェンスが潜在的にあらゆる仕事を置き換えることができる点まで向上し続ける未来を考慮して、どのように関連性を維持するかについて同僚と多くの議論をしてきました。私の個人的な戦略は専門化です。背景を持たない難しい分野では AI を扱うことができないことに気づいたからです。たとえば、友人からの量子物理学の博士論文を理解するためにクロード・オプスを使用しようとしました。私のバックグラウンドはエンジニアリングで、その後はコンパイラ/静的解析です。 opus はその背後にある大まかなアイデアを得るのに大いに役立ちましたが、私の知識不足によりいくつかの問題が見えました。
- opus の説明が正しいか間違っているかを確認できませんでした
・AIに説明を簡略化してもらうにしても、吸収すべき前提知識が多すぎてわざわざ出力する意味がなかった
- 博士号からどのような予測ができるかを理解するために AI と協力するのは非常に困難でした。モデルは多くの出力を生成できましたが、その答えはよくわかりませんでした。メンタルモデルが欠けているように感じたため、フォローアップの質問をしても問題は実際には解決されませんでした。私の考えでは、LLM インテリジェンスがどれだけ成長しても、本質的に複雑な分野では、人々はその成果を理解し、評価し、使用するために依然として専門知識を必要とするでしょう。また、人間が出力を理解せず、すべてを委任するような未来が最も効率的であるとも私は疑っています。マシンがユーザーの利益に合わせて調整されているかどうかを理解するのはどちらも困難だからです。したがって、専門化は将来に向けて良い戦略のように思えます。周囲の意見も聞きたい

この話題。あなたも同じことを観察したことがありますか?他の経験やデータに基づいて同意しませんか?

## Original Extract

I've had a lot of discussions with peers about the possibility of us being replaced via ai agents and how to stay relevant, given a future where AI intelligence keeps improving to the point of being capable of replacing potentially any job. My personal strategy is specialization, because I've noticed that I cannot work with AI in hard domains that I haven't got a background in. For example, I've tried to use claude opus to understand a phd thesis in quantum physics from a friend. My background is in engineering and later compilers/static analysis. Despite opus helping a lot to give me the high level idea behind it, I could see a few problems with my lack of background:
- I couldn't verify if opus was right or wrong in its explanations
- Even when asking the AI to simplify explanations, there was so much prerequisite knowledge I needed to absorb that there was little point to bother with its output
- It was very hard to collaborate with the AI to understand what predictions can be made from the PhD. The model could produce a lot of output but I didn't really understand its answers. Asking follow-up questions did not really solve the issue because it felt like I was missing a mental model. My thinking is that, no matter how much LLM intelligence grows, for sectors with inherent complexity, people will still need specialized expertise to understand, evaluate and use their outputs. I also doubt that the most efficient future is one where humans don't understand the outputs and delegate everything, because it will be both be hard to understand if the machines are aligned to the benefit of their users. So specialization sounds like a good strategy for the future. I'd like to hear some opinions around this topic. Have you observed the same? Do you disagree based on other experiences/data?

