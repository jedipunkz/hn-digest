---
source: "https://venturebeat.com/ai/claude-code-turned-every-engineer-into-three-now-companies-need-more-product-thinkers"
hn_url: "https://news.ycombinator.com/item?id=48778176"
title: "AI turned every engineer into three. Now companies need more product thinkers"
article_title: "Claude Code turned every engineer into three. Now companies need more product thinkers | VentureBeat"
author: "gmays"
captured_at: "2026-07-03T19:02:35Z"
capture_tool: "hn-digest"
hn_id: 48778176
score: 1
comments: 0
posted_at: "2026-07-03T18:24:02Z"
tags:
  - hacker-news
  - translated
---
...
- HN: [48778176](https://news.ycombinator.com/item?id=48778176)
- Source: [venturebeat.com](https://venturebeat.com/ai/claude-code-turned-every-engineer-into-three-now-companies-need-more-product-thinkers)
- Score: 1
- Comments: 0
- Posted: 2026-07-03T18:24:02Z

## Translation

タイトル: AI はすべてのエンジニアを 3 人に変えました。今、企業はより多くの製品思考者を必要としています
記事のタイトル: クロード コードはすべてのエンジニアを 3 人に変えました。今、企業はより多くの製品思考者を必要としています。ベンチャービート
説明: AI がビルドを圧縮しました。基礎はそれ以上に重要であり、それ以上に重要であり、プロダクトファネルは今やエンジニアが生計を立てる場所となっています。

記事本文:
クロード・コードはすべてのエンジニアを 3 人に変えました。今、企業はより多くの製品思考者を必要としています。 VentureBeat オーケストレーション
ニュースレター クロード コードはすべてのエンジニアを 3 人に変えました。今、企業はより多くの製品思考者を必要としています
Anthropic は最近、成長チームにプロダクト マネージャーを減らすのではなく、より多く雇用するように指示しました。業界報道で報告されているように、その理由は、Claude Code が社内のエンジニアリング組織を実際の人員の約 3 倍の人数で出荷するチームに密かに転換し、ボトルネックが統合開発環境 (IDE) から、何を構築するかを決定する人々に移ったためです。
その詳細は、AI の生産性に関するあらゆる主張の喧騒の中で見落とされがちです。これは、業界の残りの部分が現在直面している構造的な変化でもあります。ソフトウェアのボトルネックは入力ではなくなりました。何を入力するかを決定することです。そしてそれを他人事として扱うエンジニアは頭打ちになりつつある。
過去 10 年間のほとんどの間、その決定は他の誰かに委ねられていました。ソフトウェア エンジニアリングは、ゆっくりと吸収し、長く予測可能な手順で練習する技術です。テクノロジを深く掘り下げ、コードを書き、スタック オーバーフローに行き詰まった場合はスタック オーバーフローに質問し、スタック オーバーフローが失敗した場合は上級エンジニアにエスカレーションし、チケットを発送します。プロダクト マネージャーがファネルを所有していました。エンジニアがビルドを所有していました。双方ともこの区分を物理学として扱いました。
その後、漏斗は 5 段階で崩壊しました。
エンジニアの 1 日がどのように短縮されたかについての短い歴史
スタック オーバーフロー時代 (2014 年から 2022 年後半): エンジニアの考え方が 1 か所に集中していました。しかし、Stack Overflow での毎月の新しい質問は、2022 年 11 月以来約 77% 減少していますが、これは ChatGPT が開始されたときの偶然ではありません。撤回はサイト上の住民投票ではない。これは、それが表すワークフローに関する国民投票です。
ブラウザタブ時代 (2022 年後半から 2020 年にかけて)

24): 最初の ChatGPT 世代は IDE の外にありました。エンジニアは、より高速なオラクルを使用して、これまで実行してきたのと同じループを実行しました。ブラウザーでプロンプトを作成し、その回答を VS Code に貼り付けて戻し、これを繰り返しました。作業は依然としてシングルスレッドであり、エンジニア主導でした。影響力は本物でしたが、局地的なものでした。
IDE ネイティブ時代 (2024 年から 2025 年): カーソルとクロード コードはモデルをエディター内に移動し、完全なリポジトリにアクセスできるようにしました。上級エンジニアのエスカレーション パスはほぼ解消されました。長年にわたり、ベテラン エンジニアの間では、Bash はスタック内のツールの中で最も長い保存期間をもつという通説が広まっていました。 2026 年までに、多くの現役開発者にとって、新しいターミナルで最初に入力されるコマンドは claude になるでしょう。
仕様主導の時代 (2025 年から 2026 年): コンテキスト ウィンドウが大きくなったことで、以前はチケット、設計ドキュメント、スプリントが必要だった単一セッションの作業が行われました。 Amazon の Kiro IDE チームは、出荷していたものと同じ仕様主導のワークフローを使用して、機能ビルドを 2 週間から 2 日に短縮したと報告されています。 AWS エンジニアリングチームは、当初 30 人のエンジニアを対象とした 18 か月にわたる再アーキテクチャが、6 人によって 76 日間で完了したと説明しました。コードの作成にかかる時間がボトルネックになることはなくなりました。チームが正しいものとは何かをどれだけ明確に説明できるかが重要でした。
ルーチンの時代 (2026 年): Anthropic は 4 月に Claude Code Routines を出荷しました。定期的に、Webhook 上で、またはラップトップが閉じている間に夜間に実行される、スケジュールされた永続的なエージェントです。クロンが帰ってきた。フックが戻ってきました。エンジニアの仕事の一部はオーケストレーションになりました。就寝前に swarm を起動し、朝にはプル リクエストのスタックをレビューします。 OpenClaw のようなサードパーティ ラッパーは、部分的に復活する前に 4 月に Anthropic によって一時的に停止されましたが、オープンソース側から同じ点を指摘しました。
T

彼はボトルネックを移動させた。ほとんどのチームはそうではありません
エンジニアリングは約 3 倍になりました。製品管理は緩んでいません。 PM とエンジニアの伝統的な 1:8 の比率は、すでに緊張していましたが、各エンジニアが 1 日あたりより多くの出荷を行うため、現在は実質 1:20 に近づいています。たとえば、LinkedIn はアソシエート プロダクト マネージャーのトラックを、製品、設計、エンジニアリング全体にわたるゼネラリストを訓練する「プロダクト ビルダー」プログラムに置き換えました。 Anthropic は PM を減らすのではなく、より多く雇用します。このパターンは、実稼働環境にエージェント ワークフローを実際に展開している企業全体で一貫しています。システムは、何を構築すべきかについての決定を下すよりも、構築された機能を生成する方が早いのです。
エンジニアにとって、これはこの 10 年間で最も重要なキャリアのシグナルであり、生産性の話がフィードを独占している間、最も見逃しやすいシグナルです。
第一原則はそれ以上に重要であるが、それ以上に重要である
エージェントの時代には基本が時代遅れだと宣言する本能が、傾向をまったく間違っています。
午前 3 時にメモリ リークによって本番環境が停止し、その原因が 4 年前にプッシュされた微妙な所有権のバグであることが判明したとき、現在稼働しているエージェントはそのループをエンドツーエンドで閉じることはできません。実際のインシデントを誰が解決できるかは、オペレーティング システム、ネットワーク、同時実行性、クエリ プランによって決まります。また、エージェントの出力が表面的には正しいように見えても、裏では静かに、コストをかけて間違っている瞬間を誰が発見できるかも決定します。最新のリポジトリにコードの 70% を記述したエージェントは、スレッド セーフ、メモリ所有権、トランザクション分離に関する前提がランタイムからどこに乖離しているかを誰にも確実に伝えることができません。差分を読んでキャッチできるエンジニアは、その部屋のチームの残りのメンバーが必要とするエンジニアであり、そのエンジニアはスキルを促すことではなく、基礎に基づいて構築されます。
当然のことながら、ファンダメンタルズは今や重要です。

衛生上のスキルではなく、日常的なスキルです。 2014 年には、TCP 再送信がどのように機能するかを知ることで、デバッグ チケットをより早くクローズできるようになりました。 2026 年には、同じ知識により、エージェント主導のリリース パイプライン全体が大規模な回帰を回避できます。下で何が起こっているかを知っているエンジニアの爆発半径は、下ではなく増加しました。
2026 年のエンジニアは、注意深く読むことができる速度を超える速度でコードを生成します。迅速に出荷して生き残るチームは、AI が生成したコードのレビューを、少なくともかつてコードを作成するために確保していたものと同じ厳密さでエンジニアが扱うチームです。 2025 年の Stack Overflow 開発者調査では、開発者の 84% が AI ツールを使用しており、そのうち 46% が出力を信頼していないと回答し、前年の 31% から大幅に増加しました。このギャップ、つまり、頻繁に使用されることと信頼性が低いことは、まさにレビュー スキルが現在最も重要な点です。多くのことを押し付けてレビューをほとんどしないプログラマーは、最初の実際のインシデントの際に返済期限が来る借金を蓄積しています。それを返済できるエンジニアは、その量と関連するシステムに関する深い第一原理知識を組み合わせた人です。
新たな差別化要因はプロダクトファネルです
その両方が必要です。どちらでも十分ではありません。 2026 年に重要なエンジニアは、Jira チケットの形でファネルが到着するのを待つのをやめたエンジニアです。
これは、その役割が歴史的にスキップすることを許可されていたことを実行することを意味します。
顧客と話す。実際に製品を使用している様子をご覧ください。サポートキューを読み取ります。営業電話に同席してください。製品チームが 3 層の要約を通じて得たシグナルを、エンジニアは午後 1 日で直接得ることができるようになりました。
単なる見積もりではなく、アイデアを生み出します。これまで 8 人のエンジニアのアイデアを調達していたプロダクト マネージャーは、同じ忠実度で 20 人のエンジニアのアイデアを調達することはできません。検証され、範囲が限定された案件を持って現れるエンジニアは、

もうPMの仕事をしません。エンジニアは新しい比率で必要な仕事をしています。
顧客から逆算して作業します。 Amazon は 20 年間、最初にプレスリリースを作成してきました。この規律は、1 人のチームにも、エージェントの群れにもよく適用されます。どちらも、コードを記述する前に「顧客の利益」が何を意味するのかを明確に表明することなく、誤った方向に動作するソフトウェアを大量に作成します。
帯域幅の後ろに隠れるのはやめましょう。 「このアイデアを実現する能力はありますか?」に対する正直な答え。以前は「いいえ」でした。ルーチン、フック、および協調的なエージェント スタックを使用すると、正直な答えは「アイデアの価値は何ですか?」に近くなります。それは別の会話であり、顧客に関する本当の視点がなければ会話ははるかに困難です。
上記の 5 段階の歴史は実際にはツールの歴史ではありません。それは人間が仕事のどの部分をしなければならなかったのかの歴史です。まだ人間的な部分、そして近い将来も人間的な部分は、入力からレビュー、決定、サービスを提供する顧客と解決すべき問題の選択まで、ファネルの上位に移動しました。
2026 年版の優れたエンジニアは、最も多くのコードを書いたエンジニアではありません。何を構築すべきかを知っており、構築する価値があることを証明でき、システムが自らの速度で崩壊することなく出荷するためのエージェント フリートとレビュー規律を備えているのが企業です。
これを内面化したエンジニアは、今後 10 年間、ソフトウェアがこれまでに生み出した中で最も興味深い作品に取り組むことになるでしょう。チケットを待つエンジニアは、隣のエージェントがチケットを書き出すのを眺めながら時間を過ごします。
Ishan Gupta は Amazon のソフトウェア エンジニアです。
VentureBeat コミュニティへようこそ!
当社のゲスト投稿プログラムでは、技術専門家が洞察を共有し、AI、データ インフラストラクチャ、サイバーセキュリティ、その他の分野について中立的で権利を持たない詳細な情報を提供します。

企業の未来を形作る最先端のテクノロジー。
ゲスト投稿プログラムの詳細をお読みください。また、独自の記事を寄稿することに興味がある場合は、ガイドラインを確認してください。
エンタープライズ AI、データ、セキュリティのリーダー向けの深い洞察
電子メールを送信すると、利用規約とプライバシーに関する通知に同意したことになります。
入ってるよ！最新ニュースが間もなくあなたの受信箱に届きます。もっと見る
...
Claude Code turned every engineer into three. Now companies need more product thinkers | VentureBeat Orchestration
Newsletters Claude Code turned every engineer into three. Now companies need more product thinkers
...
You're in! Our latest news will be hitting your inbox soon. More
