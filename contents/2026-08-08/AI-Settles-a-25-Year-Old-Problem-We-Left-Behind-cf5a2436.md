...
hn_url: "https://news.ycombinator.com/item?id=49226444"
title: "AI Settles a 25 Year-Old Problem We Left Behind"
article_title: "Dimitris Papailiopoulos on X: \"https://t.co/cpwPlGKWpD\" / X"
author: "amichail"
captured_at: "2026-08-08T23:19:19Z"
capture_tool: "hn-digest"
hn_id: 49226444
score: 5
comments: 0
posted_at: "2026-08-08T22:22:24Z"
tags:
  - hacker-news
  - translated
---
...
- HN: [49226444](https://news.ycombinator.com/item?id=49226444)
- Source: [twitter.com](https://twitter.com/DimitrisPapail/status/2086158118354887060)
- Score: 5
- Comments: 0
- Posted: 2026-08-08T22:22:24Z

## Translation

タイトル: AI が人類が残した 25 年来の問題を解決
記事タイトル: Dimitris Papailiopoulos on X: "https://t.co/cpwPlGKWpD" / X
説明: AI は私たちが残した 25 年間の問題を解決します

記事本文:
Xに関するディミトリス・パパイリオプーロス: "https://t.co/cpwPlGKWpD" / X Post
Dimitris Papailiopoulos @DimitrisPapail AI が私たちが残した 25 年間の問題を解決
...
結果として、N×N ガウス ワイヤレス チャネルを通じて N ビットを送信すると、受信機はそれらすべてを正確に復元する必要があります。 2000 年代以降、信号対雑音比が少なくとも 2 log N である場合に情報を取得することが理論的に可能であることが知られていました。しかし、その値に到達することが知られている唯一の方法は、指数関数探索アルゴリズムでした。単純な多項式時間アルゴリズムがまったく同じしきい値で成功することが証明されました。
それについてもう少し詳しくお話しましょう。
2009 年に、私は Alex Dimakis (@AlexGDimakis) と最初の論文に取り組みました。彼はすぐに私の博士課程アドバイザーになりました (その論文のせいではありません)。
...
ブロック エラー最適受信機、別名最尤 (ML) 検出器は、受信信号を考慮して、送信された可能性が最も高いベクトルを見つけることで、この問題を正確に解決します。この場合、ML 検出は結局のところ、この基本的な離散最小値を解決することになります。

正方形の問題:
残念ながら、人生におけるすべての良い問題と同様に、ML 検出は NP 困難です。
...
上記の 2010 年の論文では、この問題についてはあまり進展がありませんでした。この分野で多くの研究が行われたにもかかわらず、私の理解する限り、この問題は 2001 年以来未解決のままです…よりドラマチックに聞こえるように、「四半世紀」とも呼ばれます。
...
ところで、この最適化は別の GU の下で行われます

その他にも、MIMO 検出、CDMA マルチユーザー検出、整数最小二乗法、格子内の最近接ベクトルなどがあります。
...
したがって、実際に証明できることが期待されていました

SD はポリタイムで実行されます。これは H&V が論文で明確に述べたものです。彼らは、Sphere Decoder の予想される複雑さの式を導き出し、チャネルとノイズを平均して、それが多項式に見えることを示しました。それが本当であれば、その疑問は解決しました。それは信じられないような結果のように思えました。
...
長年にわたって、あらゆる SNR スケールでのブロック回復が厳密に保証されているのは、まさに 1 つの多項式時間手法だったと私には思われます。2020 年のボックス緩和では、SNR が 4 log N 程度で、おそらくそれ以下ではない場合にブロックを回復することが示されました。余談ですが、このような手法を分析するために必要な確率ツールが 1990 年代に成熟したのは興味深いことです。

2010 年代後半、そのほとんどはコミュニティが移転し、すでに分散した後のことでした。
...
しかし、小さな問題がありました😊 GPT のアルゴリズムは AMP の亜種でした。私は AMP を心から嫌います。なぜなら、AMP の分析は一生理解できないからです。そこで私は、可能であれば、より単純なアルゴリズムで同じ結果を再検証するように指示しました。私

確かに、GPT は別のアルゴリズムを生成しましたが、これも直感に反するもので、これまでに使用されたのを見たことがないものでした。
一方、Fable では、私がとても気に入ったものを思いつきました。
符号付き LMMSE、その後貪欲なビット 反転。過去に紹介され、実際に実務で使われていたアルゴリズム。
しかし、別の問題がありました！ GPT によると、ファブルの証明はほとんどが間違っていましたが、修復可能でした。そこで私は、Fable が提案したアルゴリズムに固執することに決め、GPT に Fable の証明を取得して修正するよう依頼しました。そして、それは成功しました！
...
私が欲しかったのは、集中力の持続時間が短い年老いた恐竜が泣かずに物事を消化できるという証拠だけでした。
...
基本的な線形代数と確率を理解しているので、そのような手順を検証できると自信を持っています。つまり、それが私が要求する証拠のレベルです。
...
証明するのに 30 分かかり、私がそれを検証できるようになるまでに 5 日かかりました。ちょっと異常な比率ですが、それが現実です。その結果、多項式時間で最尤法が機能するときはいつでも、単純なアルゴリズムが機能します。この問題には計算と統計のギャップはありません。
証明の高度なものは何ですか?
アルゴリズムは恥ずかしいほど単純です。しかし、なぜこれが機能するのでしょうか? LMMSE に続いて丸めを行うと、ハミング距離の観点から、送信信号の消失部分内、つまり真実から o(N) 以内に収まります。
そうすれば、ステップごとの降下ゲイン (つまり、コストがどの程度改善されるか) がガウス量によって支配され、ガウス量の均一な集中により、特定のボール内のすべての非グラウンドトゥルース ベクトルが厳密に改善された効果を提供することが確立されるため、貪欲なビット反転が行き詰まる可能性はありません。
...
Dimitris Papailiopoulos on X: "https://t.co/cpwPlGKWpD" / X Post
Dimitris Papailiopoulos @DimitrisPapail AI Settles a 25 Year-old Problem We Left Behind
...
Then, greedy bit-flipping can’t get stuck because the per-step descent gains (i.e., how much the cost improves) are governed by Gaussian quantities, and their uniform concentration establishes that every non-ground-truth vector within a certain ball offers a strictly improving
