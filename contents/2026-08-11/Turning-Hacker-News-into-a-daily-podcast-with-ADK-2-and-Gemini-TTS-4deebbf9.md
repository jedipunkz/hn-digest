---
source: "https://ykdojo.github.io/awesome-agents-on-google-cloud/hn-daily-podcast.html"
hn_url: "https://news.ycombinator.com/item?id=49260481"
title: "Turning Hacker News into a daily podcast with ADK 2 and Gemini TTS"
article_title: "Turning Hacker News into a daily podcast with ADK 2, Gemini TTS, and Cloud Run jobs | Awesome Agents on Google Cloud"
author: "yoyoyosss"
captured_at: "2026-08-11T16:47:39Z"
capture_tool: "hn-digest"
hn_id: 49260481
score: 1
comments: 0
posted_at: "2026-08-11T16:10:50Z"
tags:
  - hacker-news
  - translated
---

# Turning Hacker News into a daily podcast with ADK 2 and Gemini TTS

- HN: [49260481](https://news.ycombinator.com/item?id=49260481)
- Source: [ykdojo.github.io](https://ykdojo.github.io/awesome-agents-on-google-cloud/hn-daily-podcast.html)
- Score: 1
- Comments: 0
- Posted: 2026-08-11T16:10:50Z

## Translation

タイトル: ADK 2 と Gemini TTS を使用してハッカー ニュースを毎日のポッドキャストに変える
記事のタイトル: ADK 2、Gemini TTS、Cloud Run ジョブを使用して Hacker News を毎日のポッドキャストに変える | Google Cloud 上の優れたエージェント
説明: Google Cloud で AI エージェントを構築するためのツール、ドキュメント、リソースの厳選されたリスト

記事本文:
Google Cloud 上の優れたエージェント
ADK 2、Gemini TTS、Cloud Run ジョブを使用して Hacker News を毎日のポッドキャストに変える
あらゆる主張を情報源と照らし合わせて事実確認する NotebookLM スタイルの番組
私は Hacker News を読むのが好きですが、いつもたくさんのことが起こっています。私はすぐに最新情報を得る方法が欲しかったのです。NotebookLM のようなフレンドリーな 2 人のホスト スタイルで、過去の日のトップニュースを要約した短い毎日のポッドキャストです。ファクトチェック ループを備えているため、自分が聞いている内容が信頼できることがわかります。
それで私はそれを作りました。これは、毎朝 Cloud Run ジョブを実行するオープンソース システムです。過去 26 時間のハッカー ニュースを読み取り、話題にする価値のある記事を選択し、2 ホスト スクリプトを作成し、実際の記事やコメント スレッドと照らし合わせてファクトチェックし、音声をレンダリングして、エピソードをポッドキャスト フィードに公開します。これは、システムを通じてエンドツーエンドで生成された実際のエピソードです。
データ: Algolia HN API。キーなし、10,000 リクエスト/時間、items/<id> は 1 回の呼び出しでストーリーの完全なコメント ツリーを返します。記事のテキストはプレーン HTTP で取得されますが、それが機能しない場合に備えていくつかのフォールバックが行われます。
オーケストレーション : Cloud Scheduler によってトリガーされる、Cloud Run ジョブの ADK 2 グラフ ワークフロー。
モデル: gemini-3.1-pro-preview と gemini-3.6-flash はエピソードを生成するために一緒に使用され、gemini-3.1-flash-tts-preview は音声用です。
公開: MP3 と生成された feed.xml を含むパブリック Cloud Storage バケット。
執筆時点のバージョン: google-adk 2.6.2 、 google-genai 2.17.0 。
パイプラインは ADK 2 グラフ ワークフローです。これの優れた点は、複数ステップのワークフローをグラフとして記述でき、ノード内でモデル呼び出しが何度発生しても、順序付け、分岐、ループを含む構造が決定的なコードのままであることです。どのように機能するかを実証するために、このシステムを一から構築してみましょう

ステップバイステップで上がっていきます。完全なコードはここにあります。
最も単純なバージョンは、ソースをモデルにフィードし、ワンショットでエピソードを書き込みます。
ワークフロー = ワークフロー (
名前 = "hn_digest" ,
エッジ = [( "START" , fetch_stories , write_episode )],
)
これは、原材料を取得し、それをスクリプトに変換するという 2 段階のシーケンスを宣言します。 fetch_stories は、Hacker News API からその日のトップ記事を取得するプレーンな Python 関数です。 write_episode はエージェントであり、命令を含むモデル呼び出しであり、これらのストーリーを 2 つのホストのスクリプトに変換します。タプル チェーンはシーケンスであり、各ノードの戻り値が次のノードの入力になります。
番組を構成するストーリーを厳選したい場合はどうすればよいでしょうか?関心事の分離: キュレーションに独自のノードを与えます。
エッジ = [( "START" , fetch_stories , curate , write_episode )]
同じチェーン、もう 1 つのリンク: 取得されたストーリーはキュレーション ステップを通過し、話す価値のある少数のストーリーが選択され、それらだけがライターに送られます。
キュレーションの仕組み: キュレーターはストーリーのメタデータを JSON (タイトル、ポイント、コメント数) として取得し、そのエピソードに 7 ～ 10 のストーリーを選択するように求めるプロンプトを取得します。そのうちのいくつかはメインセグメントとして、残りはクイックメンションとして、多様性と話の面白さを最適化します。その出力は構造化されたピックのリストに強制的に組み込まれ、下流のノードはそれらのストーリーのみを処理します。
事実確認についてはどうですか？もう 1 つのノード。
エッジ = [( "START" , fetch_stories , curate , write_episode , fat_check )]
完成したスクリプトは、そのソースと照らし合わせてチェックされます。チェッカーはスクリプトからクレームを抽出し、クレームごとに構造化された判定 (検証済みまたは不合格) をその理由を説明するメモとともに返します。それらの評決は次のステップの原料となります。
しかし、1 回のパスでは問題が見つかるだけです。それらを修正するには、ルーターを追加します。

スクリプトを送り返すことができます。
エッジ = [
( "START" 、 fetch_stories 、 curate 、 write_episode 、 fat_check 、 review )、
(レビュー , { "REWRITE" : write_episode , "PASS" : 公開 }),
】
review は小さな単純な関数であり、モデルではありません。これは、 fat_check からの判定を調べます。いずれかの要求が失敗した場合は、ルート "REWRITE" が返され、エッジの dict によってスクリプトが失敗のメモとともに write_episode に送り返されます。すべてが合格した場合は、「PASS」が返され、エピソードは公開に進みます。 review は書き換え回数もカウントし、2 回を超えるとスクリプトの送り返しを停止します。
それはすでにショーの大部分です。プロダクション グラフでは、並行して実行されるストーリーごとの作業と、失敗し続けるクレームに対する制限されたカット パスが追加されます。
ワークフロー = ワークフロー (
名前 = "hn_digest" ,
エッジ = [
( "START" 、 fetch_candidates 、 curate 、 Digest_stories 、 write_script 、
ファクトチェック、レビュールーター)、
( review_router , { "REWRITE" : write_script ,
"カット" : カット失敗 ,
"レンダリング" : render_tts }),
( カット失敗 、 ファクトチェック )、
( render_tts 、パブリッシュ)、
]、
)
ここでは 2 つの新しい点があります。 Digest_stories は、選択されたすべてのストーリーをスクリプトが作成される前に並行して処理します。CUT ルートは制限付きフォールバックです。失敗し続けるクレームは、永久にループするのではなく、スクリプトから削除されます。
並列部分はどのように機能しますか?宣言するエッジは固定ですが、話数は毎日変化するため、話ごとのレーンをエッジとして描画することはできません。代わりに、ADK では、ノードが他のノードを動的に実行できるようになります。 Digest_stories は、ストーリーごとに 1 つのダイジェストをすべて同時に実行するように ADK に要求し、結果を待ちます。
@ ノード ( rerun_on_resume = True )
async def Digest_stories (ctx : Context 、node_input : dict ) -> dict :
results = asyncio を待ちます。収集 ( * [ ダイジェストストーリー ( p ) for p in node_input [ "picks" ]])
ダイジェストストーリー

単一のストーリーを扱います。 ctx はすべてのノードが受け取るワークフロー コンテキストであり、ctx.run_node はこのノードの子として別のノードを実行します。キュレーターが選択した数に関係なく、ギャザリングはそれらすべてを同時に実行します。
ファクトチェックの設計は、いくつかのバージョンを経ました。バージョン 1 では、完成したスクリプトのみをファクトチェックしました。
バージョン 2 では、各ストーリーのレーン内にファクトチェック ステージが追加され、スクリプトが存在する前に、そのストーリー自体の記事とコメントに対してダイジェストを監査します。
同じ日のストーリーで両方を実行したところ、バージョン 2 が勝ちました。どちらのバージョンも、失敗したクレームは書き換えで修正されるか、カット パスによって削除されるため、すべてのクレームが検証されて終了しますが、バージョン 2 ではより早く問題を発見しました。監査では、脚本が書かれる前に 10 のストーリー ダイジェストのうち 6 つが修復され、脚本は 2 回ではなく 1 回の書き直しが必要になりました。結果として得られたエピソードは、最初のバージョンが曖昧な要約に頼っていたところ、帰属された逐語的な引用で述べられた内容にも根拠を与えました。
バージョン 3 では対称性が閉じられます。ストーリーごとのチェックはスクリプト レベルのチェックと同じ方法でループし、失敗した監査は修復のために返送され、上限は 2 ラウンドです。
スクリプトを音声に変換する
パイプラインは、完成したスクリプトを約 90 秒のセグメントに分割し、セグメントごとに 1 回 TTS モデルを呼び出し、最後に音声を連結することで、スクリプトを音声に変換します。エピソード全体に対して 1 回の通話が行われますが、実際には機能しません。最初の数分後に音声品質が低下します。各セグメントの呼び出しは次のようになります。
インタラクション = クライアント。相互作用。作成(
モデル = "gemini-3.1-flash-tts-preview" ,
入力 = TTS_STYLE + トランスクリプト 、
response_format = { "タイプ" : "オーディオ" },
世代設定 = { "音声設定" : [
{ "スピーカー" : "ハッカー" 、 "音声" : "デスピナ" },
{ "スピーカー" : "ニュース" 、 "音声" :

"カロン" },
]}、
)
ホストは公然とハッカーとニュースという名前のロボットであり、番組は「おはようございます。私はハッカーです」というギャグの暴露で始まります。 / 「そして私はニュースです。」マルチスピーカー Gemini TTS は、Google がまさにこの NotebookLM スタイルのポッドキャスト オーディオ用に位置付けた API であり、NotebookLM のオーディオ概要とは異なり、音声の上流にあるものはすべてユーザーが制御できます。
Cloud Run ジョブは実行から完了までのコンテナです。開始してグラフを 1 回実行して終了し、その間には料金は発生しません。スケジューリングは 1 つのコマンド ペアです。
gcloud run jobs create hn-digest --image $IMAGE --task-timeout 3600 ...
gcloud スケジューラ ジョブ create http hn-digest-Morning \
--schedule = "0 6 * * *" --time-zone = "アメリカ/ロサンゼルス" \
--uri = "https://run.googleapis.com/v2/projects/ $PROJECT /locations/ $REGION /jobs/hn-digest:run" \
--oauth-サービス アカウント-電子メール = $SERVICE_ACCOUNT
最初のコマンドはパイプラインをジョブとしてデプロイし、2 番目のコマンドは Cloud Scheduler に毎朝 6 時にパイプラインをトリガーするように指示します。
パブリッシュ ノードは mp3 をアップロードし、iTunes 名前空間タグ (番組のカバー アートとカテゴリ、各エピソードの mp3 へのリンク、ID、日付、および再生時間) を含む feed.xml : RSS 2.0 を再生成します。パブリック バケット上のファイルは完全なポッドキャストです。 Pocket Cast、Overcast、または「URL で番組をフォロー」オプションを備えた Apple Podcasts など、URL で番組を追加できるポッドキャスト アプリにフィード URL を貼り付けます。
ディレクトリのリストは別です。フィードを Apple Podcasts Connect と Spotify for Creators に 1 回送信します。
ストーリーの取得からフィードの公開まで、パイプラインの完全なソースはここから入手できます。

## Original Extract

A curated list of tools, docs, and resources for building AI agents on Google Cloud

Awesome Agents on Google Cloud
Turning Hacker News into a daily podcast with ADK 2, Gemini TTS, and Cloud Run jobs
A NotebookLM-style show that fact-checks every claim against its sources
I like reading Hacker News, but there is always a lot going on. I wanted a way to catch up quickly: a short daily podcast summarizing the past day’s top stories in a friendly two-host style, like NotebookLM, with fact-checking loops so that I know I can trust what I’m hearing.
So I built one. It’s an open source system that runs a Cloud Run job every morning: it reads the last 26 hours of Hacker News, picks the stories worth talking about, writes a two-host script, fact-checks it against the actual articles and comment threads, renders the audio, and publishes an episode to a podcast feed. Here is a real episode , generated end to end through the system.
Data : the Algolia HN API . No key, 10,000 requests/hour, and items/<id> returns a story’s full comment tree in one call. Article text is fetched with plain HTTP, with a few fallbacks in case that does not work.
Orchestration : an ADK 2 graph workflow in a Cloud Run job, triggered by Cloud Scheduler.
Models : gemini-3.1-pro-preview and gemini-3.6-flash together for generating the episode, gemini-3.1-flash-tts-preview for voice.
Publishing : a public Cloud Storage bucket with the mp3s and a generated feed.xml .
Versions at time of writing: google-adk 2.6.2 , google-genai 2.17.0 .
The pipeline is an ADK 2 graph workflow . The nice thing about it is that you can describe a multi-step workflow as a graph, and the structure, including the ordering, branching, and loops, stays deterministic code no matter how many model calls happen inside the nodes. To demonstrate how it works, let’s build this system from the ground up, step by step. The complete code is here .
The simplest version feeds sources to a model and writes an episode in one shot:
workflow = Workflow (
name = "hn_digest" ,
edges = [( "START" , fetch_stories , write_episode )],
)
This declares a two-step sequence: get the raw material, then turn it into a script. fetch_stories is a plain Python function that pulls the day’s top stories from the Hacker News API. write_episode is an agent, a model call with instructions, which turns those stories into a two-host script. A tuple chain is a sequence, and each node’s return value becomes the next node’s input.
What if we want to curate which stories make the show? Separation of concerns: give curation its own node.
edges = [( "START" , fetch_stories , curate , write_episode )]
Same chain, one more link: fetched stories now pass through a curation step, which picks the handful worth talking about, and only those flow on to the writer.
The way curation works: the curator gets the story metadata as JSON (titles, points, comment counts) and a prompt asking it to pick 7 to 10 stories for the episode, a few as main segments and the rest as quick mentions, optimizing for variety and how interesting they are to talk about. Its output is forced into a structured list of picks, and downstream nodes process only those stories.
What about fact-checking? One more node.
edges = [( "START" , fetch_stories , curate , write_episode , fact_check )]
Now the finished script gets checked against the sources it came from. The checker extracts the claims from the script, then for each claim it returns a structured verdict, verified or failed, with a note explaining why. Those verdicts are the raw material for the next step.
But a single pass only finds problems. To fix them, add a router that can send the script back:
edges = [
( "START" , fetch_stories , curate , write_episode , fact_check , review ),
( review , { "REWRITE" : write_episode , "PASS" : publish }),
]
review is a small plain function, not a model. It looks at the verdicts from fact_check . If any claim failed, it returns the route "REWRITE" , and the dict in the edges sends the script back to write_episode together with the failure notes. If everything passed, it returns "PASS" and the episode moves on to publishing. review also counts the rewrites, and after two it stops sending the script back.
That is already most of the show. The production graph adds per-story work that runs in parallel, and a bounded cut path for claims that keep failing:
workflow = Workflow (
name = "hn_digest" ,
edges = [
( "START" , fetch_candidates , curate , digest_stories , write_script ,
fact_check , review_router ),
( review_router , { "REWRITE" : write_script ,
"CUT" : cut_failed ,
"RENDER" : render_tts }),
( cut_failed , fact_check ),
( render_tts , publish ),
],
)
Two things are new here. digest_stories processes every chosen story in parallel before the script is written, and the CUT route is a bounded fallback: claims that keep failing get removed from the script rather than looping forever.
How does the parallel part work? The edges you declare are fixed, but the number of stories changes every day, so the per-story lanes cannot be drawn as edges. Instead, ADK lets a node run other nodes dynamically. digest_stories asks ADK to run one digest per story, all at the same time, and waits for the results:
@ node ( rerun_on_resume = True )
async def digest_stories ( ctx : Context , node_input : dict ) -> dict :
results = await asyncio . gather ( * [ digest_story ( p ) for p in node_input [ "picks" ]])
digest_story handles a single story. ctx is the workflow context that every node receives, and ctx.run_node runs another node as a child of this one. The gather runs all of them concurrently, however many the curator picked.
The fact-checking design went through a few versions. Version one fact-checked only the finished script:
Version two adds a fact-check stage inside each story’s lane, auditing the digest against that story’s own article and comments before the script exists:
I ran both on the same day’s stories, and version two won. Both versions end with every claim verified, because failed claims either get fixed in a rewrite or removed by the cut path, but version two caught problems earlier. The audits repaired 6 of the 10 story digests before the script was written, and the script then needed one rewrite instead of two. The resulting episode also grounded what it said in attributed verbatim quotes, where the first version leaned on vague summaries.
Version three closes the symmetry: the per-story check loops the same way the script-level check does, with failed audits sent back for repair, capped at two rounds:
Turning the script into speech
The pipeline turns the finished script into speech by splitting it into segments of roughly 90 seconds, calling the TTS model once per segment, and concatenating the audio at the end. A single call for the whole episode goes through, but it does not really work: the audio quality degrades after the first few minutes. Each segment call looks like this:
interaction = client . interactions . create (
model = "gemini-3.1-flash-tts-preview" ,
input = TTS_STYLE + transcript ,
response_format = { "type" : "audio" },
generation_config = { "speech_config" : [
{ "speaker" : "Hacker" , "voice" : "Despina" },
{ "speaker" : "News" , "voice" : "Charon" },
]},
)
The hosts are openly robots named Hacker and News, and the show opens with the disclosure as a gag: “Good morning. I’m Hacker.” / “And I’m News.” Multi-speaker Gemini TTS is the API Google positions for exactly this NotebookLM-style podcast audio, and unlike NotebookLM’s Audio Overviews, everything upstream of the voices is yours to control.
Cloud Run jobs are run-to-completion containers: start, run the graph once, exit, pay for nothing in between. Scheduling is one command pair:
gcloud run jobs create hn-digest --image $IMAGE --task-timeout 3600 ...
gcloud scheduler jobs create http hn-digest-morning \
--schedule = "0 6 * * *" --time-zone = "America/Los_Angeles" \
--uri = "https://run.googleapis.com/v2/projects/ $PROJECT /locations/ $REGION /jobs/hn-digest:run" \
--oauth-service-account-email = $SERVICE_ACCOUNT
The first command deploys the pipeline as a job, and the second tells Cloud Scheduler to trigger it every morning at six.
The publish node uploads the mp3 and regenerates feed.xml : RSS 2.0 with the iTunes namespace tags - cover art and category for the show, and for each episode a link to its mp3, an ID, a date, and a duration. That file on a public bucket is a complete podcast. Paste the feed URL into any podcast app that can add a show by URL, like Pocket Casts, Overcast, or Apple Podcasts with its “Follow a Show by URL” option.
Directory listing is separate: submit the feed once to Apple Podcasts Connect and Spotify for Creators.
The full source for the pipeline, everything from fetching stories to publishing the feed, is available here .
