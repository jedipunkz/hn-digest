---
source: "https://ludic.mataroa.blog/blog/ai-mania-is-eviscerating-global-decision-making/#fnref:7"
hn_url: "https://news.ycombinator.com/item?id=49291784"
title: "AI Mania Is Eviscerating Global Decision-Making"
article_title: "AI Mania Is Eviscerating Global Decision-Making — Ludicity"
author: "pavel_lishin"
captured_at: "2026-08-13T21:35:24Z"
capture_tool: "hn-digest"
hn_id: 49291784
score: 2
comments: 0
posted_at: "2026-08-13T20:59:25Z"
tags:
  - hacker-news
  - translated
---
...
- HN: [49291784](https://news.ycombinator.com/item?id=49291784)
- Source: [ludic.mataroa.blog](https://ludic.mataroa.blog/blog/ai-mania-is-eviscerating-global-decision-making/#fnref:7)
- Score: 2
- Comments: 0
- Posted: 2026-08-13T20:59:25Z

## Translation

タイトル: AIマニアが世界的な意思決定を骨抜きにしている
記事のタイトル: AI マニアが世界的な意思決定を骨抜きにする — 愚かさ

記事本文:
愚かさ
AIマニアが世界的な意思決定を骨抜きにしている
注: これは、より権威があるように見える形式で誰かと共有することに何らかの用途があると思われる場合に備えて、私の会社のブログにクロスポストされています。ここにリンクしてください。
私は、現在、企業全体が深刻な AI 精神病に陥っており、それについて彼らと合理的な会話をすることは不可能であると強く信じています。深く尊敬する個人的な友人も含まれているため、特定の人の名前は言えませんが、このことがどうなるのか心配です。
– ミッチェル・ハシモト氏、HashiCorp および Ghostty で有名
過去 1 年間、私は会社のすべての売上を達成し、2 件を除くすべての取り組みで技術的な要素を主導し、このブログの存続期間中、世界中の専門家と約 300 件の近況報告を行ってきました。これは、ニッチなサービス産業の現場の人々からフォーチュン 500 企業の幹部に至るまで多岐にわたります 1 。このため、私は民間部門と公的部門の両方の集団的機関が息を呑むような集団精神病に陥っているのを最前列で見てきました。このエッセイは、現在起こっている奇妙な力学を説明する試みであり、私は狂気に対して口先だけで幸福を保てるかどうかに左右されないという稀な立場にいるので、このような状況の中で生き残ろうとしている人々に、彼らは狂っていないと安心させようとするものである。
現実には、責任者には計画がないか、頭を下げている以外に進むべき道が見当たらないのです。銀行でも病院でも政府機関でもありません。世界の組織は、泡立つ興奮の渦中にいる人々と、現在常に恐怖とフラストレーションが入り混じった状態で暮らしている正気の人々によって捕らえられています。
I. AI 投資は一般的に完全な失敗に終わる
読み方

彼は、エージェント ワークフローのインターフェイスを提供することに軸足を置いた部門で働いていたとき、私たちがエージェント向けに作った製品に触れたことがあるユーザーはわずか 10 人しかいなかったことに気づき、再びエージェント ワークフローのサポートに軸足を移すことになりました。現在、どの企業もエージェント的なことをしなければならず、その分野でできることは 4 つほどしかないため、競争が激しいのです。身が引き締まる思いです。
企業は実際に AI 導入による生産性の大幅な向上を実感しているのでしょうか?この卑劣な事件には意味があるのだろうか？
これは簡単な質問のはずですが、これに直接答えるのは驚くほど難しいです。自分の会社がおかしくなったと報道陣に語った幹部は、すぐにその地位を追われることになるだろう。正直な従業員はすぐに解雇されたり、「無作為に」解雇されたりすることになるでしょう。実際、AI プロジェクトの成功率を難読化し、誤って伝えることは、この分野のほぼすべての関係者 (取締役会、経営陣、従業員、ベンダー、コンサルタント) の利益になります。多くの上場企業が AI の生産性の向上に関する発表を行っていますが、企業が Copilot ライセンスを購入して勝利を宣言しただけであることは事実です。
しかし、これらのプロジェクトが順調に進んでいるのかどうかを知る必要があります。ビジネス戦略の中核的理念として AI に全面的に重点を置くことが妥当な割合で成功しているのであれば、相対的なリスクと報酬について議論するのは当然です。
残念ながら、私たちは暗いタイムラインの中に生きています。私たちがチームとして観察した AI プロジェクトはすべて失敗しています。どれも – 1 年半で成功率は 0% でした。これは、参加を依頼されたプロジェクトだけでなく、まったくの無知な作業をしながら通りすがりに観察したプロジェクトでも同様です。

遅れた仕事。たとえ AI ツールが特定のワークロードを高速化することを認めたとしても、現在の投資の方法と規模は無意味です。多くの場合、失敗は AI 自体に関連しているのではなく、むしろ企業がソフトウェア プロジェクトを効果的に実行することが根本的に苦手であることが原因です。以前にも述べたように、AI プロジェクトは通常のプロジェクトのすべての失敗モードにさらされ、さらにすべてを正しく実行しても、手法の目新しさのために失敗する可能性があります。追加のリスクプロファイルを許容できるほどソフトウェアの出荷に優れている企業はほとんどありません。
しかし、多くの場合、それは LLM が達成できることの実際の失敗です。これの最も一般的なバージョンは、世界中の企業に展開されており、社内向けのチャットボットです。より大胆な企業の場合は、顧客向けのチャットボットです。話はいつも同じです。前者については、企業内部からの大幅な吸収を見たことがありません。従業員が社内チャットボットを使用しないのは、企業がドキュメントの品質が低い傾向があり、LLM は超能力を持たないためです。LLM は、書き留められてアクセス可能になったものしか認識できません。後者の顧客向けアプリケーションについては、医療予約中のライブ文字起こしを除いて、消費者として楽しい経験はほとんどありません。組織全体を中心に据える価値のあるものとはほとんど言えません。どちらの場合も、プロジェクト リーダーは、ツールが実際に使用されているかどうかなどの基本的な指標の追跡を避けるよう細心の注意を払っているか、簡単に利用できる指標を追跡しています。
たとえば、私が最後に消費者とやり取りしたのは、自動車の故障後に三菱自動車に助けを求めようとしたときでした。そこでは、非常に礼儀正しいロボットが私に問題について説明するよう求め、誰かが対応でき次第折り返し電話をすると言われました。

ブル。これは、私が実際に見たこの種のプロジェクトの実装の中で最も有能なものでした。音声は自然に聞こえ、反応が早く、本番環境で明らかに「生きている」ものであり、迅速な解決が約束されていました。
それは半年前のことですが、実際、折り返しの電話はありませんでした。
三菱から折り返しの電話がなかったのですが、何が起こったのですか?その要求はただ無効になり、その年の事件が 1 件減ったというだけでしょうか?電話ボットは人間の介入を必要とせずに私のクエリを解決したように見えますか?私たちが知っているのは、それがエラーとして表示されなかったということだけです。そうでなければ、電話がかかってきていたでしょう。あらゆる点で素晴らしく見えると確信していますが、重要な点以外は、車を購入する予定だったので、もう一台は買わないことにしたということです。
このため、私たちのチームは、業務中はいかなる状況であっても進行中の AI プロジェクトについては何も質問しないことをすぐに学びました。プロジェクトが開始された時点では、経営陣にとっては手遅れであり、必然的に危機点に達するまで介入は不可能です。考えられるポジティブな結果はありません。失敗率が非常に高いため、基本的な調査さえも耐えられない状況に陥っています。どのように進んでいるのか、目的は何なのか、誰がそれを使用しているのかといった一貫した質問は、何に対しても適切な答えがないため、作業を担当する指揮系統に対する不用意な攻撃となります。まれに、対話者が物事は順調に進んでいると述べた場合でも（通常、プロジェクトがまだ飛行中で、失敗が顕在化する機会がない間）、通常、その運命にあるのは明らかですが、少なくともそのような場合には、単純に同意し、家に帰って枕に向かって6時間連続で叫び続けることができます 3 。
これらすべては、私がほぼすべての

「AI の生産性が大幅に向上する」という企業の主張は、まったくの事実ではありません。たとえ一部の企業が明らかな利益を上げているとしても、これは例外であり、標準的なものではありません。この前提があれば、実際に起こっている力学と、多くの組織が長期 (または短期) の健康にとって実際に重要な事柄に焦点を当て続けることがどのように不可能になっているかについて話すことができます。
AI が問題の解決策ではない、ましてや企業全体の戦略の唯一の焦点では​​ない可能性さえ提起することは、まったく危険になっています。
私たちがこれまで観察してきたすべての十分に大規模なビジネス (たとえば、従業員が 500 人以上) では、継続的な進歩とますます継続的な雇用には、当該ビジネスに対する AI の変革力を信じて専門職を繰り返すことが必要になり始めていることに気づきました。私はビジネスで AI を使用する方法についてのアイデアを提供することについて話しているのではありません。つまり、宗教的職業、信仰の宣言を意味します。圧倒的に、こうした発言は技術者ではない人によって行われていますが、技術者が気に入らないように錯乱した発言をすることも珍しくありません。
私は、誰かが何のこともなく「AI がすべてを変えている」とほぼ一言一句口走ったのを何度か見たことがありますが、その後すぐに、その組織では現在 LLM を何にも使用しておらず、実際、ChatGPT (多くの場合無料枠) を利用している以外に変わったことは何一つ挙げることができないと認めました。極端なケースとして、収益が 20 億ドルを超える組織の AI を中心とした技術戦略を作成した直後に、ある幹部が、ChatGPT や AI ツールを人生で一度も使用したことがないと告白するのを見たことがあります。
当初、これらの発言は非常に不合理なものでした

彼らの顔を見て、私はそれが思想的リーダーの地位を獲得するための皮肉な策略だと思いました、そして確かにこれをしている人もいます - 私はそれを認めました。しかし、より広範な現実はさらに悪いことです。テクノロジーの背景がまったくない人々が、実際に彼らの言うことを信じています。一般的なルールとして、嘘つきとビジネスに関わることは避けるべきですが、どうしても必要な場合は、たとえプライベートな場であっても、少なくとも説得することができます。真の信者は、私利私欲による誘導さえも受け付けないため、はるかに脅威的です。
私の信念のターニングポイントは、莫大な資金を賭けている人が、LLMなしでそのパフォーマンスを達成したために最高のパフォーマンスを発揮しているのを見たことでした。雇用主が AI イノベーションについて公に語るとき、私たちは彼らが単に市場や顧客を操作しようとしているだけなのか自問する必要があります。彼らが、その戦略を外部のクライアントに伝えようとせず、私財を賭けてこのような戦略に非公式にコミットしているときは、彼らが本気で言っているとしか思えません。
少し前に、私は「AI に関するコントラ プタチェクのひどい記事」を書きましたが、その内容は、プタチェク自身のエッセイ「私の AI 懐疑論者の友人はすべて頭がおかしい」の中でのプタチェクの指摘の多くが内部的に矛盾しているという事実に焦点を当てていました 4 。しかし、問題の核心部分では、私たちは実際に全面的に同意している。なぜなら、彼はエッセイを次のように始めているからである。
技術幹部らはLLMの採用を義務付けている。それは悪い戦略です。
つまり、LLM の正確な有用性に関する議論を完全に回避することができ、非常に単純な状況に残されています。かなり反対の見解からこの問題に取り組んでいる私とプタチェクの両者にとって、人々がこの問題やその組織について本当に本当に愚かであることは完全に明らかです。

ステーションは専門スタッフに奇妙なワークフローの制約を要求しています。 5
これらの義務は、非常に奇妙な場所をもたらしました。私の同僚の何人かは現在、自分の仕事を「AIウォッシュ」しています。つまり、管理チームが満足するまで完全に有能に仕事を遂行できているとしても、マネージャーはエンジニアが業務でAIを使用していないと不満を抱くと言いました…そのため、彼らは現在、LLMが適切なツールではないと専門的に判断した状況であっても、LLMを使用していると嘘をついています。彼らは、何十年も同じように仕事をし、クロードがやったと言うだけです。他の人たちは「トークンリーダーボード」でAIの請求額を測られているが、高ければ高いほど良い、というのは私は明らかに地獄の底に落ちて、悪魔たちが絶対的なバカの入念な印象を与えて私を苦しめるからだ、だからシステム最適化を実行するという異常な能力のために雇われた人々は当然のことをしているのだ。彼らは、誰かがトークンの消費を検査し、その後 Netflix を視聴した場合に備えて、LLM が半ばもっともらしいループ内でプロンプトを表示するように設定します。たとえ出力が展開に適していないと彼ら自身が評価したとしても、誰一人捕まりませんでした。
Go リポジトリの並列コピーをチェックアウトし、作業中に AI にすべてを Zig で書き直すように指示します
...
Checking out a parallel copy of our Go repository and telling the AI to rewrite the whole thing in Zig while I wor
